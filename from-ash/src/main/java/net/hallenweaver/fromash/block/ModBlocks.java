package net.hallenweaver.fromash.block;
...
public class ModBlocks {
    ...
    private static void registerBlockItem(String name, Block block) {
        RegistryKey<Item> key = RegistryKey.of(RegistryKeys.ITEM, Identifier.of(FromAsh.MOD_ID, name));
        BlockItem item = new BlockItem(block, new Item.Settings().registryKey(key));
        Registry.register(Registries.ITEM, key, item);
    }

    public static void registerModBlocks() {
        FromAsh.LOGGER.info("Registering mod blocks for " + FromAsh.MOD_ID);

        ItemGroupEvents.modifyEntriesEvent(ItemGroups.FUNCTIONAL).register(entries -> {
           entries.add(MUD_BRICK_FURNACE);
        });
    }
}
